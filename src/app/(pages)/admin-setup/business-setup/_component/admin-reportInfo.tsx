...
import { useGeneralInformationUpdate, useReportInformation } from "@/hooks/mutate";
import {
  useCountriesData,
  useFilingReportData,
  useIndustryData,
    ...
    <div className="lg:w-[80%] w-full">
      ...
      <div className="grid md:grid-cols-2 gap-x-10 gap-y-6">
        <div className="space-y-2 ">
          ...
          <p className="font-medium">{data.fiscal_year_end}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="font-medium">{data.tax_year_end || "N/A"}</p>
        </div>

        <div className="space-y-2">
          <h3 className="text-sm text-foundation-grey-grey-900">Business Type</h3>
          <p className="font-medium">{data.business_type || "N/A"}</p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="font-medium">{data.filing_information}</p>
        </div>
...
const AdminReportInformation: React.FC<reportInfoProps> = ({ onNext, onPrev }) => {

  const [isEditMode, setIsEditMode] = useState(false);
  const [isLoading, setIsLoading] = useState(true);
  const [businessId, setBusinessId] = useState<number | null>(null);
  const [businessData, setBusinessData] = useState<reportInfoData | null>(
    null
  );
