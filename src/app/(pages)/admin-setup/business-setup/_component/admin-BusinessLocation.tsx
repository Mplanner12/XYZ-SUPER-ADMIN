...
import AdminFullBusinessLocationTable from "./admin-BusinessLocationFullTable";
...
const AdminBusinessLocation: React.FC<BusinessLocationProps> = ({
  onNext,
  onPrev,
}) => {
  ...
  const [rows, setRows] = useState<BusinessLocationUpdate[]>([]);
  ...
  useEffect(() => {
    if (data) {
      const fetchedRows = data.map(
        (item: BusinessLocationUpdate, index: number) => ({
          id: index + 1,
    ...
    <>
              ...
              >
                <thead className="bg-foundation-grey-grey-300/25 text-foundation-black-black-400 text-sm w-full border-b-foundation-grey-grey-300 text-left no-scroll">
                  <tr>
                    ...
                    <th className="py-4 px-4 font-medium w-[20% ]">Region</th>
      ...
      <div>
        {selectedTab === "full-business-location-table" && (
          <AdminFullBusinessLocationTable handleTabChange={handleTabChange} />
        )}
      </div>
    </>
  );
...
export default AdminBusinessLocation;
