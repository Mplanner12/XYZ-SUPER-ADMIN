...
const PayBillsTable = () => {
  ...
  const columns = [
    ...
    columnHelper.accessor('discountUsed', {
      header: 'Discount Used',
      cell: (info) => info.getValue(),
      footer: ({table}) => table.getFilteredRowModel().rows.reduce((acc, val)=>{
        acc+= Number(val.getValue('discountUsed'))
        return acc
      },0)
    }),
    columnHelper.accessor('creditUsed', {
      ...
      cell: (info) => info.getValue(),
      footer: ({table}) => table.getFilteredRowModel().rows.reduce((acc, val)=>{
        acc+= Number(val.getValue('creditUsed'))
        return acc
      },0)
    }),
    columnHelper.accessor('amountPay', {
      header: 'Amount to Pay (GBP)',
      cell: (info) => info.getValue(),
      footer: ({table}) => table.getFilteredRowModel().rows.reduce((acc, val)=>{
        acc+= Number(val.getValue('amountPay'))
        return acc
      },0)
    }),
  ];
	...
	return (
      ...
      <p className='text-primary-normal text-base mt-6'>Clear Selection</p>
		</div>
	);
};
