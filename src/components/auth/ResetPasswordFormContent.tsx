...
export default function ResetPasswordFormContent() {
  ...
  useEffect(() => {
    // This effect runs after searchParams are resolved on the client
    if (searchParams && !email) {
      toast.error(
        "Email not found in URL. Please request a password reset again."
      );
  ...
  // If email is not yet available (because searchParams are still resolving),
  // Suspense fallback (LoadingOverlay) from the parent will be shown.
  // Once email is resolved, if it's still null/empty, the useEffect will redirect.
  // This specific check might become redundant due to Suspense and useEffect,
  // but can be kept as an additional safeguard if needed after testing.
  if (searchParams && !email && !isPending) {
    // This state should ideally be handled by the useEffect redirecting.
    // Or, if you want to show a specific message here before redirect:
    // return <div>Verifying email parameter...</div>;
    // For now, let the useEffect handle the redirect.
    // The parent Suspense will show LoadingOverlay until searchParams are resolved.
  }
        ...
        <div className="w-full flex flex-col md:flex-row justify-center items-start lg:gap-x-4 rounded-xl shadow-lg bg-foundation-black-black-500/80 backdrop-blur-md">
          <div className="lg:max-w-[520px] flex h-auto w-full flex-col justify-between items-start text-start px-8 xl:px-16 py-2 md:py-0">
            ...
            >
              ...
              {/* ... (rest of the form fields: newPassword, confirmNewPassword) ... */}
              {/* Make sure to include the newPassword and confirmNewPassword fields here similar to the code field */}
