...
const CustomerDropdown: React.FC<DropdownProps> = ({
  ...
  const selectedOption = options?.find(option => option.customer_list_name === value);
  const buttonText = selectedOption ? `${selectedOption.customer_list_name}` : placeholder ;
    ...
    <div className={cn('relative text-base', className)} ref={dropdownRef}>
      ...
      {isOpen && (
        ...
        >
          {options?.map((option) => (
            <li
              key={option.customer_list_name}
              className={cn(
                'px-4 py-2 cursor-pointer hover:bg-gray-100',
                itemClassName
              )}
              onClick={() => handleOptionSelect(option.customer_list_name)}
            >
              {option.customer_list_name}
            </li>
          ))}
        </ul>
      )}
